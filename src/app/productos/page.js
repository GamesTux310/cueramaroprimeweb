...
export default function ProductosPage() {
  ...
  // Novedad: Ocultar banner de stock bajo
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      ...
      if (sessionStorage.getItem('hideStockBanner') === 'true') {
  ...
  const productosFiltrados = (productos || []).filter(producto => {
    ...
    const coincideCategoria = filtroCategoria === 'Todas' ||
      (producto.categoria && filtroCategoria && producto.categoria.toLowerCase() === filtroCategoria.toLowerCase());
  ...
  const getCategoriaColorClass = (categoria) => {
    ...
    if (cat === 'RES') return styles.catRes;
    if (cat === 'CERDO') return styles.catCerdo;
    return '';
  };
  ...
  return (
    <>
      ...
      <main className="main-content">
        ...
        {(productosStockBajo > 0 && mostrarAlertaStock) && (
          <section className={styles.alertSection}>
            <div className={styles.alert} style={{ position: 'relative' }}>
              ...
              <button
                onClick={() => {
                  setMostrarAlertaStock(false);
                  sessionStorage.setItem('hideStockBanner', 'true');
                }}
                style={{
                  position: 'absolute',
                  top: '12px',
                ...
                onMouseOver={(e) => e.currentTarget.style.background = 'rgba(251, 191, 36, 0.6)'}
                onMouseOut={(e) => e.currentTarget.style.background = 'rgba(251, 191, 36, 0.3)'}
              >
                ✕
              </button>
            </div>
                ...
                <div key={producto.id} className={styles.productCard}>
                  ...
                  <div className={styles.productInfo}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <span className={`${styles.productCategory} ${getCategoriaColorClass(producto.categoria)}`}>{producto.categoria}</span>
                  ...
                  {productosFiltrados.length === 0 ? (
                    ...
                    productosFiltrados.map((producto) => (
                      <tr key={producto.id}>
                        ...
                        <td><span className={`${styles.categoryBadge} ${getCategoriaColorClass(producto.categoria)}`}>{producto.categoria}</span></td>
                        <td>
